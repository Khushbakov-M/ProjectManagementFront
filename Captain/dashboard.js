...
async function fillStats() {
    ...
    const employmentData = (await axios.get(`${api_url}/myrequests/captain/5`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
    })).data;
    // console.log(employmentData);
    busy_count.innerHTML = `<strong>${employmentData.filter(item => item.status == 'accepted').length}</strong> Qabul qilingganlar`
}
